...
def train(data_paths, model_dir):
  logger.info('Running `solution_centralized.fit` function')
  solution_centralized.fit(**data_paths, model_dir=model_dir)
...
def main(root_dir='local_run'):
  ...
  train_data_paths = get_data_paths(data_path / 'train')
  ...
  ### Training
  train(train_data_paths, model_dir)
...
if __name__ == '__main__':
  root_dir = Path(f'local_run/scratch/centralized-{time.strftime("%Y-%m-%d--%H-%M-%S")}')
  logger.info(f'Using {root_dir=} (creating if not existing)')
